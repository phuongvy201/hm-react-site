...
const productService = {
  ...
  getProductsByCategory: (slug, page, perPage) => {
    return httpAxios.get(`/categories/${slug}/products?page=${page}&per_page=${perPage}`);
  },
  getAllProducts: (page, perPage) => {
    return httpAxios.get(`/products?page=${page}&per_page=${perPage}`);
  },
  search: (keyword, page, perPage) => {
    return httpAxios.get(`/products/search?keyword=${keyword}&page=${page}&per_page=${perPage}`);
  }
};
